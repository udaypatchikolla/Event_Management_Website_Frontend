...
//! Search input ----------------------------------------------------------------
  ...
  return (
    <div>
      <header className='sticky top-0 z-20 bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-b border-gray-100 flex py-2 px-6 sm:px-6 justify-between place-items-center'>
          
          <Link to={'/'} className="flex items-center gap-2">
            <img src="../src/assets/logo.png" alt="Planora" className='w-26 h-9'/>
            <span className="hidden sm:block font-extrabold text-primarydark text-lg tracking-wide">Planora</span>
          </Link>
          <div  className='hidden md:flex bg-white rounded-full py-2.5 px-4 w-1/3 gap-3 items-center shadow-md shadow-gray-200 ring-1 ring-gray-100'>
            ...
            <div ref={searchInputRef} className='flex-1'>
              <input type="text" placeholder="Search events, organizers..." value={searchQuery} onChange={handleSearchInputChange} className='text-sm text-black outline-none w-full '/>
            </div>
          ...
          {searchQuery && (
          <div className="p-2 w-144 z-10 absolute rounded left-[28.5%] top-14 md:w-[315px] md:left-[17%] md:top-16 lg:w-[540px] lg:left-[12%] lg:top-16 bg-white shadow-lg ring-1 ring-gray-100 max-h-80 overflow-auto">
            ...
            {events
              ...
              .map((event) => (
                <div key={event._id} className="p-2 rounded hover:bg-background">
                  {/* Display event details */}
                  <Link to={"/event/" + event._id}>
                      <div className="text-black text-sm sm:text-base w-full">{event.title}</div>
                  </Link>
                </div>
              ))}
          </div>
          ...
          <div className='hidden lg:flex gap-5 text-xs text-gray-700'>
          <Link to={'/'}>
            <div className='flex flex-col place-items-center py-1 px-3 rounded cursor-pointer hover:text-primarydark hover:bg-white hover:shadow-sm shadow-gray-200 hover:transition-shadow duration-1500'>
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 py-1">
                <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12l9.75-9.75L21.75 12" />
                <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 9.75V21h6.75v-6h1.5v6H19.5V9.75" />
              </svg>
              <div>Home</div>
            </div>
          </Link>
        ...
        {!!user &&(
          <div className="flex flex-row items-center gap-2 sm:gap-6 ">
            <Link to={'/useraccount'} className="flex items-center gap-2 group">
              <div className="h-8 w-8 rounded-full bg-primary text-white flex items-center justify-center font-bold text-xs">
                {(user.name || "U").slice(0,2).toUpperCase()}
              </div>
              <span className="hidden sm:block font-semibold group-hover:text-primarydark transition-colors">{user.name.toUpperCase()}</span>
            </Link>
        ...
        {!user &&(
          ...
          {!!user &&(
            ...
            <div className="absolute z-10 mt-64 flex flex-col w-48 bg-white right-2 md:right-[160px] rounded-lg shadow-lg ring-1 ring-gray-100"> 
            {/* TODO: */}
              <nav className={`block ${isMenuOpen ? 'block' : 'hidden'} `}>
                <div className="flex flex-col font-semibold text-[16px]">
                <Link className="flex hover:bg-background hover:shadow py-2 pt-3 pl-6 pr-8 rounded-lg" to={'/'} >
                  Home
                </Link>
